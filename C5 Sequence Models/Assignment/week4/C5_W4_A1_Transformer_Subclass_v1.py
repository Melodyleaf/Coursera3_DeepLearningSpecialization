...
def get_angles(pos, k, d):
    ...
    angles = pos/np.power(10000,2*i/d) #MYNOTE: Wrong for 2i!
    # END CODE HERE
...
def positional_encoding(positions, d):
    ...
    angle_rads = get_angles(np.arange(positions)[:, np.newaxis],
                            ...
                            d) #MYNOTE: See Excercise 1
  
    # apply sin to even indices in the array; 2i
    angle_rads[:, 0::2] = np.sin(angle_rads[:,0::2]) #MYNOTE: apply sin to itself
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask):
        ...
        #MYNOTE: above attention_output call xxxmask like EX3: scaled_dot_product_attention(q, k, v, mask)
...
class Encoder(tf.keras.layers.Layer):
   ...
   def call(self, x, training, mask):
       ...
       for i in range(self.num_layers):
           x = self.enc_layers[i](x,training,mask) #MYNOTE: check doc , layer(x,training,mask)
       # END CODE HERE
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, look_ahead_mask, padding_mask):
        ...
        Q2 = self.layernorm2(mult_attn_out2 + Q1)  # (batch_size, target_seq_len, fully_connected_dim)
                
        #BLOCK 3
        # pass the output of the second block through a ffn
        ffn_output = self.ffn(Q2)  # (batch_size, target_seq_len, fully_connected_dim)
        
        # apply a dropout layer to the ffn output
        ffn_output = self.dropout_ffn(ffn_output)
        
        # apply layer normalization (layernorm3) to the sum of the ffn output and the output of the second block
        out3 = self.layernorm3(ffn_output + Q2)  # (batch_size, target_seq_len, fully_connected_dim)
        # END CODE HERE
