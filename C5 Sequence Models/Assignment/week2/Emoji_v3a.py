...
def sentence_to_avg(sentence, word_to_vec_map):
    ...
    for w in words:
        ...
        if w in word_to_vec_map:
            avg += word_to_vec_map[w] #MYNOTE:  Extracts the w in words, which is GloVe representation of each word.
            # Increment count
            count +=1
          
    if count > 0:
        # Get the average. But only if count > 0
        avg = avg/count #MYNOTE: ?
...
def model(X, Y, word_to_vec_map, learning_rate = 0.01, num_iterations = 400):
    ...
    for t in range(num_iterations): # Loop over the number of iterations
        ...
        for i in range(m):          # Loop over the training examples
            ...
            avg = sentence_to_avg(X[i], word_to_vec_map)#MYNOTE: USE ex1 to return avg
            ...
            cost += -np.sum(Y_oh[i] * np.log(a)) #MYNOTE: no need to retieve ith from a, where a is a vector
            ### END CODE HERE ###
...
def pretrained_embedding_layer(word_to_vec_map, word_to_index):
    ...
    for word, idx in word_to_index.items():
        emb_matrix[idx, :] = word_to_vec_map[word]  #MYNOTE: word_to_vec_map is a dictionary,"word" is key
...
def Emojify_V2(input_shape, word_to_vec_map, word_to_index):
    ...
    sentence_indices = Input(shape=input_shape, dtype='int32') #MYNOTE: check keras documentation "tf.keras.Input"
