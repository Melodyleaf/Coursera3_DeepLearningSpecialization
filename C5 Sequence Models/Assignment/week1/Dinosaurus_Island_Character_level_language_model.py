...
def sample(parameters, char_to_ix, seed):
    ...
    x = np.zeros((vocab_size,1)) #MYNOTE: initialize to 0
    ...
    while (idx != newline_character and counter != 50):
        ...
        idx =  np.random.choice(range(len(y.ravel())), p = y.ravel()) #MYNOTE: y.ravel() is 1D vector
...
def model(data_x, ix_to_char, char_to_ix, num_iterations = 35000, n_a = 50, dino_names = 7, vocab_size = 27, verbose = False):
    ...
    for j in range(num_iterations):
        ...
        single_example_ix = [char_to_ix[c] for c in single_example_chars] #MYNOTE:  find idx reponding to chars
