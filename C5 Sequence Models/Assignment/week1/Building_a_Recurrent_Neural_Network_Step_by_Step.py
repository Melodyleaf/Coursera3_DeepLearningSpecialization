...
def rnn_forward(x, a0, parameters):
    ...
    a = np.zeros((n_a,m,T_x)) #MYNOTE: np.zeros is 2 ()!!!!
...
def rnn_backward(da, caches):
    ...
    dba = np.zeros((n_a, 1))
    da0 = np.zeros((n_a, m))
    da_prevt = np.zeros((n_a, m)) #MYNOTE: same as da_prev,"at" in Figure 7 from left
    
    # Loop through all the time steps
    for t in reversed(range(T_x)):
        # Compute gradients at time step t. Choose wisely the "da_next" and the "cache" to use in the backward propagation step. (≈1 line)
        gradients = rnn_cell_backward(da[:,:,t] + da_prevt, caches[t])#MYNOTE: Figure 7 from right to left
...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    dcct = (dc_next*it + ot*(1-np.power(np.tanh(c_next),2))*it*da_next) * (1-np.power(cct,2))
    dit = (dc_next*cct + ot*(1-np.power(np.tanh(c_next),2))*cct*da_next) * it *(1-it)
    dft = (dc_next*c_prev + ot*(1-np.power(np.tanh(c_next),2))*c_prev*da_next) * ft *(1-ft)
    
    #MYNOTE: below is also ok
    #dot = da_next * np.tanh(c_next) * ot * (1 - ot)
    ...
    dWf = np.dot(dft,(np.concatenate((a_prev,xt))).T) #MYNOTE ok: dWf = np.dot(dft, np.hstack([a_prev.T, xt.T]))
    dWi = np.dot(dit,(np.concatenate((a_prev,xt))).T) #MYNOTE ok: dWi = np.dot(dit, np.hstack([a_prev.T, xt.T]))
    dWc = np.dot(dcct,(np.concatenate((a_prev,xt))).T) #MYNOTE ok: dWc = np.dot(dcct, np.hstack([a_prev.T, xt.T]))
    dWo = np.dot(dot,(np.concatenate((a_prev,xt))).T) #MYNOTE ok: dWo = np.dot(dot, np.hstack([a_prev.T, xt.T]))
    ...
    #MYNOTE: Retrieve Wf from parameters["Wf"]
...
def lstm_backward(da, caches):
    ...
    dc_prevt = np.zeros((n_a, m)) #MYNOTE: dc shape is the same as da
