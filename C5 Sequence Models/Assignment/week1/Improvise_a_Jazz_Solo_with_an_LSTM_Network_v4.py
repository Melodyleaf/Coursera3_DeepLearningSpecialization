...
def djmodel(Tx, LSTM_cell, densor, reshaper):
    ...
    for t in range(Tx):
        ...
        x = reshaper(x) #reshaper = Reshape((1, n_values))    
        # Step 2.C: Perform one step of the LSTM_cell
        a, _, c = LSTM_cell(inputs=x, initial_state=[a, c]) #MYNOTE: LSTM_cell = LSTM(n_a, return_state = True)  
        # Step 2.D: Apply densor to the hidden state output of LSTM_Cell
        out = densor(a)  #MYNOTE: densor = Dense(n_values, activation='softmax')   
...
def music_inference_model(LSTM_cell, densor, Ty=100):
    ...
    for t in range(Ty):
        ...
        x = tf.math.argmax(out,axis=1) #axis=0,return last column; axis=1,return last row.Above instruction return last axis,means axis=1
        x = tf.one_hot(x,n_values) #MYNOTE: n_values=90,shape of x. Example: tf.one_hot(indices,depth) depth=n_values
        # Step 2.E:  
        # Use RepeatVector(1) to convert x into a tensor with shape=(None, 1, 90)
        x = RepeatVector(1)(x)
        
    # Step 3: Create model instance with the correct "inputs" and "outputs" (≈1 line)
    inference_model = Model(inputs=[x0, a0, c0], outputs=outputs) #MYNOTE: initial state are x0,a0,c0
...
def predict_and_sample(inference_model, x_initializer = x_initializer, a_initializer = a_initializer, 
    ...
    indices = np.argmax(np.array(pred), axis=-1) #axis=-1,return to last axis of array
    # Step 3: Convert indices to one-hot vectors, the shape of the results should be (Ty, n_values)
    results = to_categorical(indices, num_classes= 90) #MYNOTE: x_initializer.shape =(1, 1, 90)
    #results = to_categorical(indices, num_classes= x_initializer.shape[2])
    #MYNOTE hard coding: results = to_categorical(indices, num_classes= 90) 
    ### END CODE HERE ###
