...
def triplet_loss(y_true, y_pred, alpha = 0.2):
    ...
    pos_dist = tf.reduce_sum(tf.square(tf.subtract(anchor,positive)),axis=-1) #MYNOTE: axis=-1
...
def verify(image_path, identity, database, model):
    ...
    dist = np.linalg.norm(encoding-database[identity]) #MYNOTE: call identity NO '' !!
