...
def compute_content_cost(content_output, generated_output):
    ...
    #a_C_unrolled = tf.reshape(a_C, shape=[1, -1, n_C]) #MYNOTE: OK
    #a_G_unrolled = tf.reshape(a_G, shape=[1, -1, n_C]) #MYNOTE: OK
    # compute the cost with tensorflow (≈1 line)
    J_content = tf.reduce_sum(tf.square(tf.subtract(a_C_unrolled,a_G_unrolled))) / (4*n_H*n_W*n_C) #MYNOTE: unrolled for substract
...
def compute_layer_style_cost(a_S, a_G):
    ...
    #MYNOTE: wrong for 4*tf.square(n_C)*tf.square((n_W * n_H)) !!!!!!!
    #J_style_layer = None
...
def train_step(generated_image):
    with tf.GradientTape() as tape:
        ...
        #(1 line)
        #J_style = compute_style_cost(style_image_output, generated_image, STYLE_LAYERS=STYLE_LAYERS)
        J_style = compute_style_cost(a_S, a_G) #MYNOTE: compute all layers with landa!!! not just one layer
