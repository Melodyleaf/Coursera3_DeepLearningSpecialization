...
def zero_pad(X, pad):
    ...
    #MY NOTE: X=(m, n_H, n_W, n_C)=(m,4,4,3)
    #MY NOTE: X_pad = np.pad(X,((pad,pad), (pad,pad), (pad,pad), (pad,pad)), mode='constant', constant_values = (0,0))
    #MY NOTE: m,n_C=(0,0),because only n_H+n_W need padding
    # YOUR CODE ENDS HERE
        ...
        # for h in range(None):           # loop over vertical axis of the output volume
            ...
            # for w in range(None):       # loop over horizontal axis of the output volume
                ...
                # for c in range(None):   # loop over channels (= #filters) of the output volume
                    ...
                    # Z[i, h, w, c] = None
    # YOUR CODE STARTS HERE
    (m, n_H_prev, n_W_prev, n_C_prev) = A_prev.shape #MY NOTE1 # Retrieve dimensions from A_prev's shape
    (f, f, n_C_prev, n_C) = W.shape #MY NOTE2 # Retrieve dimensions from W's shape
    stride = hparameters["stride"] #MY NOTE3 # Retrieve information from "hparameters" 
    pad = hparameters["pad"] 
    n_H = int((n_H_prev-f+2*pad)/stride)+1 #MY NOTE4 # Compute the dimensions of the CONV output volume
    n_W = int((n_W_prev-f+2*pad)/stride)+1
    Z = np.zeros((m, n_H, n_W, n_C)) #MY NOTE5 # Initialize the output volume Z with zeros. np.zeros()(), 2 ()
    A_prev_pad = zero_pad(A_prev, pad) #MY NOTE6 # Create A_prev_pad by padding A_prev
    for i in range(m):
            ...
            # for w in range(n_W):               # loop on the horizontal axis
                # for c in range(n_C):           # loop over the channels (depth)
                    ...
                    # elif mode == "average":
                        ...
                        # dA_prev[i, vert_start: vert_end, horiz_start: horiz_end, c] += None
    # YOUR CODE STARTS HERE
    (A_prev, hparameters) = cache #MY NOTE1: # Retrieve information from cache (≈1 line)
    
    stride = hparameters["stride"] #MY NOTE2: Retrieve hyperparameters from "hparameters" (≈2 lines)
    f = hparameters["f"]
    
    m, n_H_prev, n_W_prev, n_C_prev = A_prev.shape #MY NOTE3: # Retrieve dimensions from A_prev's shape and dA's shape (≈2 lines)
    m, n_H, n_W, n_C = dA.shape
    
    dA_prev = np.zeros((m, n_H_prev, n_W_prev, n_C_prev))   #MY NOTE4: # Initialize dA_prev with zeros (≈1 line)
    
    for i in range(m): # loop over the training examples
        a_prev = A_prev[i] #MY NOTE5: # select training example from A_prev (≈1 line)
        
        for h in range(n_H):                   # loop on the vertical axis
            for w in range(n_W):               # loop on the horizontal axis
                for c in range(n_C):           # loop over the channels (depth)
        
                    vert_start = h * stride #MY NOTE6: # Find the corners of the current "slice" (≈4 lines)
                    vert_end = h * stride + f
                    horiz_start = w * stride
                    horiz_end = w * stride + f
                    ...
                    if mode == "max":
                        ...
                        # Set dA_prev to be dA_prev + (the mask multiplied by the correct entry of dA) (≈1 line)
                        dA_prev[i, vert_start: vert_end, horiz_start: horiz_end, c] += np.dot(mask,dA[i,h,w,c])
