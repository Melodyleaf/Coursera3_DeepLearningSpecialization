...
def yolo_filter_boxes(boxes, box_confidence, box_class_probs, threshold = .6):
    ...
    ##(≈ 1 line)
    box_scores = box_confidence * box_class_probs #MYNOTE: No need to randn

    # Step 2: Find the box_classes using the max box_scores, keep track of the corresponding score
    ##(≈ 2 lines)
    # IMPORTANT: set axis to -1
    box_classes = tf.math.argmax(box_scores,axis=-1)#MYNOTE:input is processing with "box_scores"
    box_class_scores = tf.math.reduce_max(box_scores, axis=-1, keepdims=False)#MYNOTE:input is processing with "box_scores"
    
    # Step 3: Create a filtering mask based on "box_class_scores" by using "threshold". The mask should have the
    # same dimension as box_class_scores, and be True for the boxes you want to keep (with probability >= threshold)
    ## (≈ 1 line)
    filtering_mask = box_class_scores>=threshold  #MYNOTE:just set threshold is enough
