...
def sigmoid(z):
    ...
    # a = ...
    # YOUR CODE STARTS HERE
    z = tf.cast(z,tf.float32) #MY NOTE: z is number, not str,do not need "z"
...
    Returns:
    ...
    #MY NOTE: (depth,) CAN BE REPLACED BY shape=[-1], where -1 means no limits in shape, 
    #MY NOTE: axis=0 means column
...
def compute_cost(logits, labels):
    ...
    # cost = ...
    # YOUR CODE STARTS HERE
    logits = tf.transpose(logits) #MY NOTE: NEED MORE CLARIFICATION
    labels = tf.reshape(tf.transpose(labels),[logits.shape[0],6]) #MY NOTE: NEED MORE CLARIFICATION
    cost = tf.reduce_mean(tf.keras.losses.categorical_crossentropy(labels,logits,from_logits=True)) #MY NOTE: NEED MORE CLARIFICATION
